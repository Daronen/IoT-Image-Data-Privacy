...
def main():
    ...
    for x in OutputIMG:
        """
        If you wish to hide other objects simply add code similar too

        if(x['lable'] == 'object'):
            InpaintPositions.append(x['position'])
        
        If you wish to change what info abput a chair will cause it to be hidden look at the ProductData_chair.txt for the info that will be returned in data.
        """
        if(x['lable'] == 'chair'):
            data = GetImageClusterV2.getImageInfo(x['image'], x['lable'])
            #print(data)
